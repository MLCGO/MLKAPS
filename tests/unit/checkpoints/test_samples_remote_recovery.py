...
class TestSimpleSrun:
    def test_simple_srun_timeout(self, remote_recovery_data):
        doSrun, target_machine = remote_recovery_data
        timelog_path = pathlib.Path(__file__).parent / "time_log.csv"
        if timelog_path.exists() and timelog_path.is_file():
            timelog_path.unlink()  # Remove the file

        py_program = str(pathlib.Path(__file__).parent / "dummy_clock.py")
        time_limit = "00:01:00"  # hh:mm:ss
        srun_cmd = ["srun", "-w", f"{target_machine}", f"--time={time_limit}", sys.executable, py_program]
        save_e = ""
        if not doSrun:
            pytest.skip("SLURM not configured")
        else:
            try:
                subprocess.run(srun_cmd, capture_output=True, text=True, check=True)
            except subprocess.CalledProcessError as e:
                save_e = e

            assert save_e.returncode != 0
            assert timelog_path.exists() and timelog_path.is_file()
            timelog = pd.read_csv(timelog_path)
            assert len(timelog) > 59  # timeout was a minute
            assert len(timelog) < 120  # make sure timeout was reasonable
...
class TestTimeoutRecovery:
    ...
    def test_can_timeout(self, remote_recovery_data):
        doSrun, target_machine = remote_recovery_data
        ...
        py_program = str(pathlib.Path(__file__).parent / "dummy_recovery_kernel.py")
        time_limit = "00:01:00"  # hh:mm:ss
        sleep_time = 120  # seconds, the recovery kernel will sleep long enough to time out
        srun_cmd = ["srun", "-w", f"{target_machine}", f"--time={time_limit}", sys.executable, py_program, f"{sleep_time}"]
        ...
        if not doSrun:
            ...
            while tries < 4:
                tries = tries + 1
                try:
                    res = subprocess.run(srun_cmd, capture_output=True, text=True, check=True)
                    if res.returncode == 0:
                        break
                except subprocess.CalledProcessError:
                    pass

            assert tries > 1, "did not time out"
